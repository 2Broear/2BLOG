<?php
    ...
    $cur_template=='default' ? include_once(TEMPLATEPATH . '/index.php') : include_once(TEMPLATEPATH . '/templates/'.$cur_template);//use category as folder
    $cats = get_categories(meta_query_categories(0, 'ASC', 'seo_order'));
    if(!empty($cats)){
        foreach($cats as $the_cat){
            $the_cat_id = $the_cat->term_id;
            $the_cat_temp = TEMPLATEPATH . '/templates/'.get_term_meta($the_cat_id, 'seo_template', true);  // 子分类继承父分类模板（获取分类绑定模板）
            ...
            if(!empty($catss)){
                foreach($catss as $the_cats){
                    ...
                    $the_cats_temp = TEMPLATEPATH . '/templates/'.get_term_meta($the_cats_id, 'seo_template', true);
                    ...
                    if(!empty($catsss)){
                        foreach($catsss as $the_catss){
                            ...
                            $the_catss_temp = TEMPLATEPATH . '/templates/'.get_term_meta($the_catss_id, 'seo_template', true);
