<?php
        ...
        if (!$output_data || count($output_data) <= 1) {
            ...
            foreach ($output_data as $data) {
                ...
                if (isset($data->child)) {
                    ...
                    $output_string .= '<details class="rest">
                        <summary> 浏览其余 '.$output_limit.' 篇文章 </summary>
                        <ol>';
                            foreach ($output_child as $child) {
                                $output_string .= '<li>
                                    <a href="'.$child->link.'" target="_blank">'.$child->title.'</a>
                                    <p class="content">'.$child->desc.'...</p>
                                    <p class="pub">Published at '.$child->date.'</p>
                                </li>';
                            }
    ...
    */
    // 扫描指定路径目录文件
    function dirScaner($rootPath, $deepScan = false, $dirOnly = false, $fileExtend = '.log', $ajaxRequest = false, $fomart = false) {
        // // make sure we are on the backend
        // if (!is_admin()) return false;
        if ($ajaxRequest) {
            $nonce = date('Ymd'); //get_request_param('_ajax_nonce');//
            // check_ajax_referer($nonce . '_dirscaner_ajax_nonce');  // 检查 nonce
        }
        $logFiles = [];
        $stack = [$rootPath];
        while (!empty($stack)) {
        ...
        if (!$ajaxRequest) return $logFiles;
