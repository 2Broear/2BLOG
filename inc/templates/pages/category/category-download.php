...
function get_download_posts($cats, $order=1){
    ...
    for($i=0;$i<$cats_count;$i++){
        ...
        if($term_order==$order){
            ...
            $output .= '<div class="dld_box fade-item '.$cat_slug.'"><div class="dld_box_wrap magnetic" data-magnet-scale="1" data-magnet-step="0.025"><div class="box_up preCover"><span style="background:url('.$cat_poster.') center center /cover"><a href="javascript:;"><h3> '.$cat_name.' </h3><i> '.strtoupper($cat_slug).'</i><em></em></a></span></div><div class="box_down"><ul>';
                //setup query
                ...
                while ($left_query->have_posts()):
                    ...
                    $output .= '<li class="'.$class_disabled.$class_topset.'"><div class="details"><a href="'.$href.'" target="'.$target.'" rel="nofollow" title="下载附件"><img '.$lazyhold.' src="'.$loadimg.'" alt="poster" /></a><div class="desc">'.get_the_title().'<a href="'.$href.'" target="'.$target.'" rel="nofollow">下载附件</a>'.$single_link.'</div></div></li>';
                endwhile;
