...
以上分类名称将作为友情链接分类判断的依据（分类别名可随意变更），通过设置 __将这个链接设为私密链接__ 来改变链接状态为 `standby` 即某个链接无法访问时可应用特殊样式（不适用于 `missing` 类型）
> 自定义排序请手动修改 `category-2bfriends.php` 文件中 `site_link` 函数的 `orderby` 参数。  
> 
> __额外属性：__ 当链接中的 __评级__ 属性 `==1` 时该链接右下角将被标记为 ___`girl`___ （♀标识），当链接中的 __评级__ 属性 `==10` 时该链接左上角将显示 `NICE ONE` 特殊标签
