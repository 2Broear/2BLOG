...
#### 文章排序
WP自带的 __“置顶这篇文章”__ 将作为 ___在首页置顶___ ，普通文章则使用文章编辑中的 `排序（列表）排序值` 进行排序（最后编辑排序值>排序值>发布日期），值越大越靠前。

#### 评论设置
在 wp 后台 __设置->讨论__ 中可设置 ___评论数量、分页、嵌套、限制等___
