...
### 分类导航
在 __分类__ 栏目中的下方有一组名为 `Page Sync Options` 的选项，里面有各分类的自定义属性，包括 __Background Images__ （分类背景）、__Page Template__ （分类绑定的页面模板）、__Page Title__ （分类/页面自定义标题）、__Page Keywords__ （分类/页面自定义关键词）、__Page Description__ （分类描述）5 个选项，其中的 ___分类背景___ 用于后台开启 __元分类导航__
后所应用的背景图， ___页面模板___ 则为创建分类后所同步的页面模板（创建一个分类后会自动创建对应title名称、slug别名的页面，在创建时可选择对应页面所需使用的页面模板） _这里隐藏了一个 —__Category Order__ 选项，也就是 __页面__ 中的 `menu_order` ，默认创建分类时会将其 `term_id` 同步到对应页面的 `menu_order` 中作为排序值，__在创建分类后，单独编辑分类时可以查看和修改其导航排序（默认规则越小越靠前，适用于所有层级）__ _
...
- __`standard`__ 默认友链（包含头像、名称、描述）
- __`special`__ 特殊友链（包含名称、描述）
- __`sitelink`__ 全站友链（包含名称、在所有页面底部显示）
- __`missing`__ 失联友链（包含名称、在 _特殊友链_ 下方显示，nofollow）

___额外属性：___
若链接中的 __评级__ 属性 `>=1` ，则该链接将被标记为 ___`girl`___ （特殊♀标识）
