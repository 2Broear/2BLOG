...
当分类别名设置为 __`slash`__ 时，其创建的分类将使用 `/` 作为分类导航别名 __（即无导航，此时访问该导航下的分类时将直接作为其父级的层级显示页面）__ （个人需求，不需要可无视）

> 注意，后台 __通用控制->页面层级关系__ 可以控制是否同步分类层级到页面层级（查看编辑页面时更清晰），此选项 __默认关闭__ ，因其开启后将会导致无法正常使用 ___`slash`___ 关键字的原因就设置为默认关闭状态了，___如果没有使用 `slash` 将子级作为父级输出的需求，则可以开启 ___
