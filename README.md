...
> 分类自带的 __描述__ 选项可在某些存在背景图的页面中作为 ___页面小标题___ 使用。
> 
...
2. 文章模板分为 `news` 和 `notes` ，默认模板为 `single-notes.php` （原因是该模板类型比较通用，也可在 `single.php` 中替换为自定义 `single-default.php` 作为默认模板），__在发布文章时无需手动选择，保留默认模板即可自动匹配。__
...
本主题遵循GPL协议开源主题，在您免费使用此主题时，__需保留站点底部右下角声明字样。__
